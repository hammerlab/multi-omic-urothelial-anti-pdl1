...
parameters {
  ...
  vector<lower=0>[M] ts_beta_sigma_sigma;
  vector[M] beta; // beta-intercept
  vector[M] raw_beta_time_sigma;
  vector[T-2] raw_ts_beta_sigma[M];
...
transformed parameters {
  ...
  vector[T-1] ts_beta_sigma[M];
  vector<lower=0>[T-1] beta_time_sigma[M];
  ...
  for (coef in 1:M) {
      ...
      ts_beta_sigma[coef][1] = 0;
      for (time in 2:T) {
          ts_beta_sigma[coef][time] = raw_ts_beta_sigma[coef][time-1];
  ...
  for (coef in 1:M) {
      ...
      beta_time_sigma[coef] = log(raw_beta_time_sigma[coef] + cumulative_sum(ts_beta_sigma[coef]));
  }
...
model {
  ...
  for (m in 1:M) {
    ts_beta_sigma[m] ~ cauchy(0, 1);
    raw_beta_time_deltas[m][1] ~ normal(0, 100);
    for (i in 2:(T-1)) {
        ts_beta_sigma[m][i] ~ normal(ts_beta_sigma[m][i-1], ts_beta_sigma_sigma[m]);
        raw_beta_time_deltas[m][i] ~ normal(raw_beta_time_deltas[m][i-1], raw_beta_time_sigma + ts_beta_sigma[m][i]);
    }
  }
  raw_beta_time_sigma ~ cauchy(0, 1);
  ts_beta_sigma_sigma ~ cauchy(0, 1);
  beta ~ cauchy(0, 1);
