...
transformed data {
  ...
  int<lower=0> s_observed_neoant_count[S];
  ...
  for (n in 1:N) {
      ...
      s_observed_neoant_count[s[n]] = observed_neoant_count[n];
...
transformed parameters {
  ...
  for (n in 1:N) {
    hazard[n] <- exp(x[n,]*beta + 
                    missense_snv_rate[s[n]]*beta_missense_neoant[1] +
                    neoant_snv_rate[s[n]]*beta_missense_neoant[2]
                    )*baseline[t[n]];
  }
}
model {
  ...
  neoant_snv_rate ~ normal(0, 1);
  beta_missense_neoant ~ normal(0, 1);
  beta ~ normal(0, 1);
  phi ~ normal(0, 1);
  ...
  s_observed_neoant_count ~ neg_binomial_2(to_vector(s_observed_missense_count) .* to_vector(neoant_snv_rate), phi);
